  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      for (var i = 0; i < forecastDays.length; i++) {
        forecastDays[i].querySelector(".forecast-date").textContent = new Date(
          data.list[forecastIndex].dt_txt
        ).toLocaleDateString();
        forecastDays[i].querySelector(
          ".forecast-icon"
        ).innerHTML = `<img src="http://openweathermap.org/img/w/${data.list[forecastIndex].weather[0].icon}.png" alt="Weather Icon">`;
        forecastDays[i].querySelector(
          ".forecast-temperature"
        ).textContent = `${data.list[forecastIndex].main.temp.toFixed(1)}°C`;
        forecastDays[i].querySelector(
          ".forecast-humidity"
        ).textContent = `Humidity: ${data.list[forecastIndex].main.humidity}%`;
        forecastIndex += 8;
      }
    });
}
